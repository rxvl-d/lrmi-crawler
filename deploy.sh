...
cat run_sbatch.sh.template | sed "s/SIF_FILE/lrmi-crawler_$1.sif/" | sed 's/N_CORES/40/' | sed 's/SOURCE/2019/' > run_sbatch.sh
scp run_sbatch.sh devbox2:run_sbatch_19.sh
rm run_sbatch.sh
cat run_sbatch.sh.template | sed "s/SIF_FILE/lrmi-crawler_$1.sif/" | sed 's/N_CORES/40/' | sed 's/SOURCE/2020/' > run_sbatch.sh
scp run_sbatch.sh devbox2:run_sbatch_20.sh
rm run_sbatch.sh
cat run_sbatch.sh.template | sed "s/SIF_FILE/lrmi-crawler_$1.sif/" | sed 's/N_CORES/40/' | sed 's/SOURCE/2021/' > run_sbatch.sh
