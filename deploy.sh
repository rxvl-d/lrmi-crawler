...
cat run_sbatch.sh.template | sed "s/SIF_FILE/lrmi-crawler_$1.sif/" | sed 's/N_CORES/20/' | sed 's/SOURCE/wdc19/' > run_sbatch.sh
scp run_sbatch.sh devbox2:run_sbatch_19.sh
rm run_sbatch.sh
cat run_sbatch.sh.template | sed "s/SIF_FILE/lrmi-crawler_$1.sif/" | sed 's/N_CORES/20/' | sed 's/SOURCE/wdc20/' > run_sbatch.sh
scp run_sbatch.sh devbox2:run_sbatch_20.sh
rm run_sbatch.sh
cat run_sbatch.sh.template | sed "s/SIF_FILE/lrmi-crawler_$1.sif/" | sed 's/N_CORES/20/' | sed 's/SOURCE/wdc21/' > run_sbatch.sh
